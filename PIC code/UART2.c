...
unsigned int Disp2String(char *str) 
{
    unsigned int i;
    unsigned int length = strlen(str);
    //XmitUART2(0x0A,2);  //LF
    // Clear terminal and return carriage before printing
    //XmitUART2(0x0D,1);  //CR
    //XmitUART2(0x20,25);
    XmitUART2(0x0D,1);  //CR 
    for (i=0; i <= length; i++)
...
//Clear terminal
void clearTerm(unsigned int length)
{
    XmitUART2(0x0D, 1);
    XmitUART2(0x20, length);
    XmitUART2(0x0D, 1);
}

int readByte(void)
{
    // if byte has already been sent, return byte
    if(IFS1bits.U2RXIF){
        IFS1bits.U2RXIF = 0;
        return U2RXREG;
    }
    ...
    // if byte is not sent before timeout, trigger reset
    if(delay_ms(3000)){
        XmitUART2(0,1);
        Reset();
    }
