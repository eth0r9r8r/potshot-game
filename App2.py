...
def main():
    ...
    while (1):
        ...
        if state == "wait":     #Wait for an input from the PIC to start
            println(" AppPro.2: The Game ")    #Display title screen
            while ser.inWaiting() < 1: pass    #Echos the received byte when it gets it
            echo = ser.read(1)    
            if echo == b'S':   #Go to stage select
                ser.write(b'S') 
                state = "select"
            elif echo == b'E':   #Exit python program
                break 
        ...
        if state == "select":       #Stage select screen
            ...
            if pressed == b'1' and level != 1:     #Decrease level if it wouldn't go below 1
                ...
                state = "waitLoad"
            elif pressed == b'\x00':   #If PIC sent 0, go back to start
                state = "wait" 
        ...
        if state == "waitLoad":
            ...
            if echo == b'L':
                ser.write(b'L')
                state = "loadLevel"
            elif echo == b'\x00':    #If PIC sent 0, go back to start
                state == "wait"
        ...
        if state == "pause":
            ...
            if echo == b'\x00':   #If PIC sent 0, go back to start
                state = "wait"
            else:
                state = "play"
